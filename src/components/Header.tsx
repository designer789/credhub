...
export default function Header() {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 w-full py-4 z-40">
      <div className="container max-w-7xl mx-auto px-4 flex justify-between items-center">
        <Link 
          href="/" 
          className="font-general font-semibold text-2xl text-foreground flex items-center gap-2"
        >
          <Image
            src="/icon.png"
            alt="Cred.Hub Logo"
        ...
        <nav className="hidden lg:flex space-x-6">
          {menuItems.map((item) => (
            <a 
              ...
              className="font-general text-gray-700 hover:text-primary transition-colors relative group"
            >
              {item.label}
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-700 group-hover:text-primary transition-colors" viewBox="0 0 252 300" fill="currentColor">
        ...
        <button 
          onClick={toggleMenu}
          className="lg:hidden ml-auto md:ml-4 flex items-center justify-center w-10 h-10 rounded-md text-gray-700 hover:bg-gray-100 focus:outline-none transition-colors z-50"
        ...
        >
          <svg 
            ...
            viewBox="0 0 24 24" 
            stroke="currentColor" 
            className={`h-6 w-6 transition-opacity duration-300 ${isMenuOpen ? 'opacity-0 absolute' : 'opacity-100'}`}
          >
          ...
          <svg 
            ...
            viewBox="0 0 24 24" 
            stroke="currentColor" 
            className={`h-6 w-6 transition-opacity duration-300 ${isMenuOpen ? 'opacity-100' : 'opacity-0 absolute'}`}
          >
      ...
      {/* Mobile Menu Panel */}
      <div 
        className={`fixed top-0 right-0 bottom-0 w-full sm:w-80 bg-white shadow-xl z-40 transform transition-transform duration-300 ease-in-out ${
        ...
        <div className="h-full flex flex-col p-6 overflow-y-auto">
            ...
            <div className="flex flex-col space-y-4">
              {menuItems.map((item) => (
                <a 
                  ...
                  href={item.href}
                  onClick={(e) => scrollToSection(e, item.href.substring(1))}
                  className="font-general text-xl text-gray-800 hover:text-primary py-2 border-b border-gray-100 transition-colors"
                >
                  {item.label}
                </a>
              ))}
          ...
          <div className="mt-auto pt-6">
            ...
            <p className="text-center mt-6 text-gray-500 text-sm">
              &copy; {new Date().getFullYear()} Cred.Hub. All rights reserved.
            </p>
          </div>
        </div>
      </div>
